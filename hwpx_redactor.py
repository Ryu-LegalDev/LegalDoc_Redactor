...
REGEX_PATTERNS = {
    ...
    "계좌번호": re.compile(r"\d{3,6}-\d{2,6}-\d{2,6}"),
    "사건번호": re.compile(r"\d{4}[가-힣]{1,3}\d{4,7}"),
...
def extract_party_roles(preview_text: str, detected_names: list[str] | None = None) -> dict[str, str]:
    ...
    # 형식: "원 고 전 송 희" 또는 "원고 김철수"
    header_pattern = re.compile(
        r"^(원 *고|피 *고) +([가-힣](?: [가-힣])* *[가-힣])",
        re.MULTILINE,
    )
    role_counters = {}
    ...
    # 본문에서 "소외 [이름]" 패턴 추출
    sooe_pattern = re.compile(r"소외\s+([가-힣]{2,4})(?:[(\s])")
    sooe_counter = 0
...
def regex_redact(text: str) -> tuple[str, list[dict]]:
    ...
    for label, pattern in REGEX_PATTERNS.items():
        for m in pattern.finditer(text):
            findings.append({"type": label, "value": m.group(), "span": m.span()})
...
- 사람이름: 실제 사람의 고유한 이름 (예: 김철수, 이영희)
  주의: "원고", "피고", "소외인", "소외", "소송대리인", "담당변호사" 등 소송 역할 호칭은 개인정보가 아닙니다. 반드시 제외하세요.
  주의: 법률 서면에서 이름에 띄어쓰기가 있을 수 있습니다 (예: "김 현 정", "전 송 희"). 이것도 사람이름입니다.
- 장소: 구체적인 고유 지명, 주소, 상호명 (예: 의정부시, 한국관나이트클럽)
  주의: "집 주소", "피고의 직장", "거주지" 등 구체적이지 않은 일반 표현은 제외하세요.
...
def redact_image(
    ...
    for bbox, matched_name in matches:
        ...
        if role_label:
            ...
            tx = x0 + ((x1 - x0) - label_w) // 2
            ty = y0 + ((y1 - y0) - label_h) // 2
...
def redact_hwpx(input_path: str, output_path: str | None = None):
    ...
    for item in llm_findings_raw:
        ...
        if key in seen or item["value"] not in full_text:
            continue
        if item["value"] in EXCLUDE_VALUES:
            continue
        if item["type"] == "장소" and "법원" in item["value"]:
            continue
        seen.add(key)
